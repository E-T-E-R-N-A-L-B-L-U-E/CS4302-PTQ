...
class QuaLinear(torch.nn.Module):
    def __init__(self, weight, bias):
        super().__init__()
        self.weight = self.bias = None
        if weight is not None:
            scale = (torch.max(weight) - torch.min(weight)) / torch.tensor(255.)
            zero_point = (-torch.min(weight)) / scale
            self.weight = torch.quantize_per_tensor(weight, scale, zero_point, dtype=torch.quint8)
        if bias is not None:
            scale = (torch.max(bias) - torch.min(bias)) / torch.tensor(255.)
            zero_point = (-torch.min(bias)) / scale
            self.bias = torch.quantize_per_tensor(bias, scale, zero_point, dtype=torch.quint8)
    
    def forward(self, x: Tensor) -> Tensor:
        x = torch.dequantize(x)
        weight = bias = None
        if self.weight is not None:
            weight = torch.dequantize(self.weight)
        if self.bias is not None:
            bias = torch.dequantize(self.bias)
        return F.linear(x, weight, bias)


class GATConvQua(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lin_weight = copy.deepcopy(lin_weight)
        self.lin_bias = copy.deepcopy(lin_bias)

        # In case we are operating in bipartite graphs, we apply separate
        # transformations 'lin_src' and 'lin_dst' to source and target nodes:
        self.lin = self.lin_src = self.lin_dst = None
        if isinstance(in_channels, int):
            self.lin = QuaLinear(self.lin_weight, self.lin_bias)
    ...
    def forward(
        ...
        # x_src = x_src.dequantize()
        # x_dst = x_dst.dequantize()
        x = (x_src, x_dst)
